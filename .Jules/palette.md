...
## 2024-05-18 - Semantic Grouping of Form Controls with Fieldset
**Learning:** Found several instances where form elements (like checkbox groups or compound inputs like eye prescription details) were visually grouped using `div` containers and styled `span` or orphaned `label` tags acting as headings. This creates accessibility barriers, as screen readers cannot associate the grouping context with the individual controls inside them.
**Action:** Always use semantic `<fieldset>` and `<legend>` elements to group related form controls, particularly for radio buttons, checkboxes, or grouped data like prescription details. For groupings that don't need a visible heading, use a `<legend class="sr-only">`. Apply Tailwind's `border-none p-0 m-0` to the `<fieldset>` and `w-full` to the `<legend>` to reset default browser styles and maintain the intended visual design.
