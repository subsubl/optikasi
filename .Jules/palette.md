...
## 2024-05-04 - Screen Reader Accessible Star Ratings
**Learning:** Repeating visual icons representing a single semantic value (like a 5-star rating) clutter screen reader output if not grouped and hidden properly.
**Action:** Wrap the elements in a container with `role="img"` and a descriptive `aria-label`, and apply `aria-hidden="true"` to the decorative children.
