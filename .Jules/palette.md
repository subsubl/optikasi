...
## 2026-07-28 - Enhance form accessibility and global disabled states
**Learning:** When implementing disabled states for forms during async submission in Vue, applying the disabled attribute to native inputs alongside Tailwind disabled variants (`disabled:opacity-50 disabled:cursor-not-allowed`) is crucial for UX and accessibility. For custom Vue checkboxes built with visually hidden native `<input class="peer">`, the disabled attribute must be applied to the hidden input, and the associated visual `<span>` must be styled using Tailwind's `peer-disabled:opacity-50 peer-disabled:cursor-not-allowed` variants to correctly reflect the state without breaking interactivity or tab order.
**Action:** Use native `:disabled` attribute bound to the global submitting state (`isSubmitting`) on all form elements (inputs, textareas, selects), styled with `disabled:opacity-50 disabled:cursor-not-allowed`. For custom checkboxes, apply `:disabled` to the hidden input and use `peer-disabled:` utilities on the proxy element.
