...
## 2026-10-28 - Screen Reader Grouping for Repeated Visual Icons (e.g. 5-star Ratings)
**Learning:** Repeating visual elements representing a single combined value (such as a 5-star rating) cause screen readers to repetitively announce each element (e.g. "star star star star star"), which creates noise and poor UX.
**Action:** Wrap the repeating elements in a container with `role="img"` and a descriptive `aria-label`, and apply `aria-hidden="true"` to the individual decorative child elements.
