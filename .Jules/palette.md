...
## 2026-07-26 - Add explicit label to email field in BookingForm
**Learning:** The booking form's email input was missing an associated `<label>` element, creating a critical accessibility gap for screen reader users and making the required field unclear.
**Action:** Added a semantically correct `<label for="email">` element that visually matches the existing design system. The required field indicator uses `aria-hidden="true"` to prevent redundant screen reader announcements while providing clear visual feedback.
