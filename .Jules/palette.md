...
**Action:** Ensure the target container of a skip link always includes `id="target-id"`, `tabindex="-1"`, and `focus:outline-none` (using Tailwind, or standard CSS) to smoothly accept and manage programmatic focus without visual disruption.## 2024-05-14 - Refactoring Custom Checkbox Groups for Accessibility
**Learning:** Visual custom checkbox groups often use orphaned `<label>` elements or visual `div`/`span` containers which lack semantic context for screen readers when navigating directly to inputs.
**Action:** When creating grouped form inputs, especially complex ones like day/time selection, wrap the parent container in a `<fieldset>` with a semantic `<legend>` to properly associate the group title with the checkboxes. Use nested fieldsets with `<legend class="sr-only">` for subgroups (e.g., separating "Weekdays" from "Time Slots") to maintain clean screen reader announcements without affecting the visual layout.
