...
**Action:** Ensure the target container of a skip link always includes `id="target-id"`, `tabindex="-1"`, and `focus:outline-none` (using Tailwind, or standard CSS) to smoothly accept and manage programmatic focus without visual disruption.## 2024-04-11 - Accessible Form Grouping and Required Fields
**Learning:** In complex forms with nested structures (like eye prescriptions or multi-checkbox time slots), relying on purely visual `div` and `span` tags for layout strips context for screen-reader users, causing them to hear isolated inputs without knowing what section they belong to. Adding asterisks visually for required fields also leads to redundant/confusing announcements if not explicitly hidden with `aria-hidden="true"`.
**Action:** Always group related inputs using semantic `<fieldset>` and `<legend>`. Reset their default browser styles using Tailwind classes (`border-none p-0 m-0 w-full block`) to maintain the existing visual design perfectly while improving underlying accessibility. Additionally, hide visual asterisks marking required fields by using `<span aria-hidden="true">*</span>`.
