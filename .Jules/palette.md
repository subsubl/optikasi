...
**Learning:** Repetitive decorative characters representing a single value (like 5 stars `★★★★★`) are read out loud by screen readers as redundant text ("Star Star Star Star Star"), leading to a poor user experience.
**Action:** Wrap such visual repetitions in a container with `role="img"` and a descriptive `aria-label` (e.g., "Ocena 5 od 5 zvezdic"), and apply `aria-hidden="true"` to the inner decorative elements to prevent redundant announcements.
