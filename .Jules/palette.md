...
**Action:** Ensure the target container of a skip link always includes `id="target-id"`, `tabindex="-1"`, and `focus:outline-none` (using Tailwind, or standard CSS) to smoothly accept and manage programmatic focus without visual disruption.## 2024-05-24 - Accessible Star Ratings\n**Learning:** When using repetitive text characters for visual indicators (like 5 star emojis or symbols: ★★★★★) for a rating, screen readers will read each character out loud sequentially (e.g., "star star star star star"). This is poor UX.\n**Action:** Wrap the characters in a container with `role="img"`, add a descriptive `aria-label` (e.g., "Rating 5 out of 5 stars"), and apply `aria-hidden="true"` to the repetitive characters or their inner container to provide a single, clear announcement to screen reader users.
