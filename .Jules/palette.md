...
## 2026-10-29 - Icon-only Links Accessibility
**Learning:** Icon-only links (like social media icons in footers) that use SVGs with `aria-hidden="true"` must include a descriptive `aria-label` on the parent `<a>` or `<button>` element to be accessible to screen readers, and must implement keyboard focus styles (e.g., `focus:outline-none focus-visible:ring-2 focus-visible:ring-accent`). Decorative textual or visual elements (e.g., '+' or '-' icons or hamburger menus) inside interactive components should be hidden from screen readers using `aria-hidden="true"`.
**Action:** Always verify that interactive elements with only graphical content have an explicit `aria-label` and `focus-visible` styles, and that decorative SVGs have `aria-hidden="true"`.
