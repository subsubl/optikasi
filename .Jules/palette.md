...
**Action:** Ensure the target container of a skip link always includes `id="target-id"`, `tabindex="-1"`, and `focus:outline-none` (using Tailwind, or standard CSS) to smoothly accept and manage programmatic focus without visual disruption.## 2026-06-09 - Visual Indicator for Required Fields
**Learning:** Required form fields (using HTML5 `required` attribute) that lack a visual indicator (like an asterisk) can confuse sighted users when form submission is blocked. Furthermore, simply adding an asterisk can negatively impact screen reader users if it's read aloud (e.g., 'Name star').
**Action:** Always include a visual indicator (like `<span aria-hidden="true">*</span>`) alongside the label text for `required` fields, ensuring it is hidden from assistive technologies to maintain a clean reading experience.
