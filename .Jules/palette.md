...
## 2026-10-29 - Required Field Indicators
**Learning:** Forms enforcing fields via the HTML5 `required` attribute lack visual indicators, which can block submission unexpectedly for users.
**Action:** Add a visual indicator (like an asterisk with `aria-hidden="true"`) alongside the label text for all required inputs to improve usability and accessibility.
