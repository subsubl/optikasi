...
**Action:** Ensure the target container of a skip link always includes `id="target-id"`, `tabindex="-1"`, and `focus:outline-none` (using Tailwind, or standard CSS) to smoothly accept and manage programmatic focus without visual disruption.## 2026-05-06 - Adding Accessible Required Indicators and Tactile Button Feedback
**Learning:** Adding a visible asterisk to required form fields improves UX by preventing validation errors, but the asterisk must be hidden from screen readers using `aria-hidden="true"` to avoid redundant announcements (since the input already has the `required` attribute). Additionally, hover states on cards provide visual feedback, but adding `active:scale-95` provides necessary tactile feedback for a complete interaction.
**Action:** When adding required asterisks, always use `<span aria-hidden="true">*</span>`. When implementing interactive cards or buttons with hover states, include `active:scale-95` to complete the click interaction feeling.
