...
**Action:** Ensure the target container of a skip link always includes `id="target-id"`, `tabindex="-1"`, and `focus:outline-none` (using Tailwind, or standard CSS) to smoothly accept and manage programmatic focus without visual disruption.## 2024-05-18 - Screen Reader Experience for 5-Star Ratings\n**Learning:** Repeating visual elements like stars (★★★★★) in a row are read by screen readers as individual, repetitive words (e.g., "star, star, star..."), which is annoying and unhelpful.\n**Action:** When encountering a visual rating component, wrap the repeating decorative elements in a `role="img"` container with a descriptive `aria-label` (e.g., "Ocena 5 od 5 zvezdic"), and apply `aria-hidden="true"` to the individual elements.
