...
## 2026-10-29 - Star Rating Accessibility
**Learning:** Visual text representing combined value, such as 5-star ratings (★★★★★ or `v-for="i in 5"`), are confusing when a screen reader reads "star star star star star".
**Action:** Wrap the combined rating elements in a parent `<div role="img" aria-label="Rating...">` and add `aria-hidden="true"` to the decorative star character tags so they are visually present but semantically combined.
