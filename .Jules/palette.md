...
## 2026-10-29 - Accessible Repeating Visual Elements
**Learning:** Screen readers announce repeating visual elements (like a 5-star rating using "★") multiple times (e.g., "star star star star star"), which is a poor user experience.
**Action:** Wrap repeating visual elements representing a single value in a container with `role="img"` and a descriptive `aria-label` (e.g., "5 od 5 zvezdic"), and apply `aria-hidden="true"` to the decorative child elements or text to hide them from assistive technologies.
