...
**Learning:** Repeating visual elements like 5-star ratings (★★★★★) are often read out individually by screen readers ("star star star star star"), which is frustrating and inefficient.
**Action:** Wrap repeating visual elements representing a single combined value in a container with `role="img"` and a descriptive `aria-label` (e.g., "Ocena 5 od 5 zvezdic"), and apply `aria-hidden="true"` to the decorative child elements to hide them from screen readers.
