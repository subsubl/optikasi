...
**Action:** Ensure the target container of a skip link always includes `id="target-id"`, `tabindex="-1"`, and `focus:outline-none` (using Tailwind, or standard CSS) to smoothly accept and manage programmatic focus without visual disruption.## 2026-05-18 - Enhanced Booking Form Loading and Validation States
**Learning:** In highly interactive Vue components tracking an `isSubmitting` state, form inputs frequently lack proper styling when the standard HTML `disabled` attribute is dynamically applied. Without styling, it isn't clear to the user that the form is locked. Additionally, screen readers redundantly read visual required symbols like "*" if they aren't hidden.
**Action:** When adding `disabled` states, use Tailwind's `disabled:opacity-50 disabled:cursor-not-allowed` on all native inputs. For custom peer-styled checkboxes, use `peer-disabled:opacity-50 peer-disabled:cursor-not-allowed`. Wrap visual indicators like asterisks in `<span aria-hidden="true">` to preserve accessibility when the corresponding input already has the HTML `required` attribute.
