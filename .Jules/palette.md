...
**Action:** Ensure the target container of a skip link always includes `id="target-id"`, `tabindex="-1"`, and `focus:outline-none` (using Tailwind, or standard CSS) to smoothly accept and manage programmatic focus without visual disruption.## 2024-05-18 - Client Testimonials Rating Accessibility
**Learning:** Repeating visual icons for a single rating (e.g., five stars) are read out repetitively by screen readers if left exposed.
**Action:** Wrap the icons in a `div` with `role="img"` and an `aria-label` describing the total rating (e.g., "Ocena 5 od 5 zvezdic"), and add `aria-hidden="true"` to the individual icons.
