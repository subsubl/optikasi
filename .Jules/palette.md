...
## 2026-07-17 - Consistent Missing Labels on Required Form Fields
**Learning:** Found that some primary form fields in this Nuxt app (like the email input in `BookingForm.vue`) are missing associated labels, which is a major accessibility issue. Also learned that relying strictly on Tailwind's `disabled:` and `peer-disabled:` utilities provides a very clean, unified way to visually disable both standard inputs and custom checkbox proxy elements without writing custom CSS.
**Action:** When auditing forms, systematically verify that every `id` on an `<input>` has a strictly matching `<label for="...">`. Add `:disabled="isSubmitting"` to all interactive fields (not just the submit button) to prevent partial submissions and maintain consistent UI state during async operations.
