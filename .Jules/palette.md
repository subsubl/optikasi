...
**Action:** Ensure the target container of a skip link always includes `id="target-id"`, `tabindex="-1"`, and `focus:outline-none` (using Tailwind, or standard CSS) to smoothly accept and manage programmatic focus without visual disruption.## 2024-05-18 - Checkbox Group Accessibility and Tailwind Peer Disabled States
**Learning:** When creating accessible groups of custom checkboxes, avoid using plain `div` elements and orphaned `label` elements as headers. Additionally, when using Tailwind's `peer` class on hidden inputs, the disabled state on the visual proxy (e.g., a `span`) must use `peer-disabled:opacity-50 peer-disabled:cursor-not-allowed` to reflect the input's disabled attribute.
**Action:** Always wrap groups of related checkboxes (and nested groups) in `<fieldset>` with an appropriate `<legend>` (using `sr-only` if a visual label isn't required for sub-groups). For custom inputs styled via Tailwind `peer`, always bind the `:disabled` property on the input to a loading state and add `peer-disabled` utility classes to the sibling visual element.
