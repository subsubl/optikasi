...
**Learning:** Repeating visual elements representing a combined value (like a 5-star rating) are announced redundantly and unhelpfully by screen readers (e.g., "star, star, star, star, star").
**Action:** Wrap the elements in a container with `role="img"` and a descriptive `aria-label` (e.g., "Ocena 5 od 5 zvezdic"), and apply `aria-hidden="true"` to the individual decorative child elements.
