...
## 2024-05-14 - Accessible Repeating Visual Elements
**Learning:** Decorative repeating visual elements representing a combined single value (like a 5-star rating using multiple '★' characters) are read out individually by screen readers (e.g., "star star star star star"), which creates a poor UX.
**Action:** Wrap the decorative elements in a container with `role="img"` and a descriptive `aria-label` (e.g., "Ocena 5 od 5 zvezdic"), and apply `aria-hidden="true"` to the decorative child elements so they are ignored.
