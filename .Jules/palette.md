...
## 2026-07-18 - Add explicit label to email input in BookingForm\n**Learning:** When adding explicit labels to required form fields, it's crucial to include both the `for` attribute mapping to the input's `id` and an `aria-hidden="true"` visual indicator (like an asterisk) to prevent screen readers from announcing the symbol while ensuring the semantic `required` attribute on the input remains intact. In Slovenian, 'E-naslov' is used for Email.\n**Action:** Always verify form inputs have explicit `<label for="...">` elements rather than just relying on surrounding `divs` or placeholders, and use `aria-hidden="true"` on decorative required indicators.
