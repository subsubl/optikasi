...
**Action:** Ensure the target container of a skip link always includes `id="target-id"`, `tabindex="-1"`, and `focus:outline-none` (using Tailwind, or standard CSS) to smoothly accept and manage programmatic focus without visual disruption.## 2026-06-20 - Adding visual required indicators to form fields
**Learning:** Relying solely on the HTML5 `required` attribute is insufficient for a good user experience because it can lead to unexpected submission blocks without clear prior indication of which fields are mandatory.
**Action:** Always include a visual indicator (like an asterisk) alongside the label text for required form fields, and ensure it is hidden from screen readers using `aria-hidden="true"` to prevent redundant announcements.
