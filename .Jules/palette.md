...
## 2026-10-29 - Repeated Visual Elements Accessibility
**Learning:** Repeating visual elements representing a single combined value (such as a 5-star rating) can be overly verbose or confusing for screen readers if read individually.
**Action:** Wrap the repeating elements in a container with `role="img"` and a descriptive `aria-label` (e.g., "Ocena 5 od 5 zvezdic"), and apply `aria-hidden="true"` to the individual decorative child elements to provide a clear, concise announcement.
