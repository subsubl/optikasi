...
## 2025-05-27 - Grouping Form Controls
**Learning:** Using generic `div` and `label` elements to group checkboxes is inaccessible as it fails to convey the relationship to screen readers.
**Action:** When grouping related form controls (like days of the week or time slots), use semantic `<fieldset>` and `<legend>` elements to explicitly define the groups. For nested visual groups, use an `sr-only` `<legend>` to maintain design consistency while providing context to assistive technologies.
