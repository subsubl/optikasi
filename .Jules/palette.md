...
**Action:** Ensure the target container of a skip link always includes `id="target-id"`, `tabindex="-1"`, and `focus:outline-none` (using Tailwind, or standard CSS) to smoothly accept and manage programmatic focus without visual disruption.## 2026-05-25 - Accessible Star Ratings

**Learning:** When using repeated visual elements like a 5-star rating (e.g. `★`), screen readers might read each star individually, which is confusing.
**Action:** Wrap the elements in a container with `role="img"` and a descriptive `aria-label` (e.g. `Ocena 5 od 5 zvezdic`), and apply `aria-hidden="true"` to the decorative child elements.
