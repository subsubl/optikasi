    ...
    <div class="bg-white p-8 md:p-12 shadow-sm border border-gray-100">
      ...
      <nav aria-label="Napredek kviza" class="flex justify-center mb-12">
        <ol class="flex items-center">
          <li v-for="(label, index) in stepLabels" :key="index" class="flex items-center">
            <div
              :class="[
                'w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold transition-colors',
                step >= index + 1 ? 'bg-accent text-white' : 'bg-gray-100 text-gray-400'
              ]"
              :aria-current="step === index + 1 ? 'step' : undefined"
            >
              <span aria-hidden="true">{{ index + 1 }}</span>
              <span class="sr-only">Korak {{ index + 1 }} od {{ stepLabels.length }}: {{ label }}</span>
            </div>
            <div v-if="index < stepLabels.length - 1" class="w-12 h-0.5 bg-gray-200" aria-hidden="true"></div>
          </li>
        </ol>
      </nav>
...
const step = ref(1)
const stepLabels = ['Oblika obraza', 'Stil', 'Proračun']
