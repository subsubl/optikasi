  ...
  <form @submit.prevent="submitForm" class="space-y-6 bg-white p-8 border border-gray-100 shadow-sm">
    ...
    <div class="grid md:grid-cols-2 gap-6">
      <div class="space-y-2">
        ...
        <input id="name" v-model="form.name" type="text" name="name" required class="w-full border-b border-gray-300 focus:border-accent outline-none py-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" placeholder="Janez Novak" :disabled="isSubmitting" />
      </div>
      <div class="space-y-2">
        ...
        <input id="email" v-model="form.email" type="email" name="email" required class="w-full border-b border-gray-300 focus:border-accent outline-none py-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" placeholder="janez@example.com" :disabled="isSubmitting" />
      </div>
    </div>

    <div class="space-y-2">
      <label for="phone" class="text-xs uppercase tracking-widest text-gray-500 font-bold">Telefonska Številka</label>
      <input id="phone" v-model="form.phone" type="tel" name="phone" class="w-full border-b border-gray-300 focus:border-accent outline-none py-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" placeholder="041 123 456" :disabled="isSubmitting" />
    </div>

    <fieldset :disabled="isSubmitting" class="space-y-4 pt-4 border-none p-0 m-0 disabled:opacity-50">
      ...
      <fieldset class="border-none p-0 m-0">
        ...
        <div class="flex flex-wrap gap-3 mb-4">
          <label v-for="day in days" :key="day" :class="['cursor-pointer', isSubmitting ? 'cursor-not-allowed' : '']">
            <input type="checkbox" :value="day" v-model="form.preferredDays" class="sr-only peer" :disabled="isSubmitting" />
      ...
      <fieldset class="border-none p-0 m-0">
        ...
        <div class="flex flex-wrap gap-3">
          <label v-for="slot in slots" :key="slot" :class="['cursor-pointer', isSubmitting ? 'cursor-not-allowed' : '']">
            <input type="checkbox" :value="slot" v-model="form.preferredSlots" class="sr-only peer" :disabled="isSubmitting" />
    ...
    <div class="space-y-2">
      ...
      <textarea id="message" v-model="form.message" name="message" rows="4" class="w-full border bg-gray-50 border-gray-200 focus:border-accent outline-none p-3 transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed" :disabled="isSubmitting"></textarea>
    </div>
