  ...
  <form @submit.prevent="submitForm" class="space-y-6 bg-white p-8 border border-gray-100 shadow-sm">
    ...
    <div class="grid md:grid-cols-2 gap-6">
      <div class="space-y-2">
        ...
        <input id="name" v-model="form.name" type="text" name="name" required class="w-full border-b border-gray-300 focus:border-accent outline-none py-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" :disabled="isSubmitting" placeholder="Janez Novak" />
      </div>
      <div class="space-y-2">
        ...
        <input id="email" v-model="form.email" type="email" name="email" required class="w-full border-b border-gray-300 focus:border-accent outline-none py-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" :disabled="isSubmitting" placeholder="janez@example.com" />
      </div>
    </div>

    <div class="space-y-2">
      <label for="phone" class="text-xs uppercase tracking-widest text-gray-500 font-bold">Telefonska Številka</label>
      <input id="phone" v-model="form.phone" type="tel" name="phone" class="w-full border-b border-gray-300 focus:border-accent outline-none py-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" :disabled="isSubmitting" placeholder="041 123 456" />
    </div>

    <fieldset class="border-none p-0 m-0 space-y-4 pt-4">
      <legend class="w-full block text-xs uppercase tracking-widest text-gray-500 font-bold">Želeni Termini (Izberite več možnosti)</legend>
