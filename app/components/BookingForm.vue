<template>
  <form @submit.prevent="submitForm" class="space-y-6 bg-white p-8 border border-gray-100 shadow-sm">
    ...
    <div class="grid md:grid-cols-2 gap-6">
      <div class="space-y-2">
        <label for="email" class="text-xs uppercase tracking-widest text-gray-500 font-bold">
          E-naslov<span aria-hidden="true" class="text-accent ml-1">*</span>
        </label>
        <input id="email" v-model="form.email" :disabled="isSubmitting" type="email" name="email" required class="w-full border-b border-gray-300 focus:border-accent outline-none py-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" placeholder="janez@example.com" />
      </div>
    </div>

    <div class="space-y-2">
      <label for="phone" class="text-xs uppercase tracking-widest text-gray-500 font-bold">Telefonska Številka</label>
      <input id="phone" v-model="form.phone" :disabled="isSubmitting" type="tel" name="phone" class="w-full border-b border-gray-300 focus:border-accent outline-none py-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" placeholder="041 123 456" />
    </div>
    ...
    <div class="space-y-2">
      ...
      <textarea id="message" v-model="form.message" :disabled="isSubmitting" name="message" rows="4" class="w-full border bg-gray-50 border-gray-200 focus:border-accent outline-none p-3 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm"></textarea>
    </div>
