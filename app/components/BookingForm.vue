<template>
  <form @submit.prevent="submitForm" class="space-y-6 bg-white p-8 border border-gray-100 shadow-sm">
    ...
    <div class="grid md:grid-cols-2 gap-6">
      <div class="space-y-2">
        <label for="name" class="text-xs uppercase tracking-widest text-gray-500 font-bold">
          Ime in Priimek <span class="text-accent" aria-hidden="true">*</span>
        </label>
        <input id="name" v-model="form.name" type="text" name="name" required :disabled="isSubmitting" class="w-full border-b border-gray-300 focus-visible:ring-2 focus-visible:ring-accent focus:outline-none py-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" placeholder="Janez Novak" />
      </div>
      <div class="space-y-2">
        <label for="email" class="text-xs uppercase tracking-widest text-gray-500 font-bold">
          Email Naslov <span class="text-accent" aria-hidden="true">*</span>
        </label>
        <input id="email" v-model="form.email" type="email" name="email" required :disabled="isSubmitting" class="w-full border-b border-gray-300 focus-visible:ring-2 focus-visible:ring-accent focus:outline-none py-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" placeholder="janez@example.com" />
      </div>
    </div>

    <div class="space-y-2">
      <label for="phone" class="text-xs uppercase tracking-widest text-gray-500 font-bold">Telefonska Številka</label>
      <input id="phone" v-model="form.phone" type="tel" name="phone" :disabled="isSubmitting" class="w-full border-b border-gray-300 focus-visible:ring-2 focus-visible:ring-accent focus:outline-none py-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" placeholder="041 123 456" />
    </div>

    <fieldset class="space-y-4 pt-4 border-none p-0 m-0" :disabled="isSubmitting">
      <legend class="text-xs uppercase tracking-widest text-gray-500 font-bold w-full block">Želeni Termini (Izberite več možnosti)</legend>

      <!-- Weekdays -->
      <fieldset class="border-none p-0 m-0">
        <legend class="sr-only">Dnevi v tednu</legend>
        <div class="flex flex-wrap gap-3 mb-4" :class="{ 'opacity-50 cursor-not-allowed': isSubmitting }">
          <label v-for="day in days" :key="day" :class="['cursor-pointer', { 'cursor-not-allowed': isSubmitting }]">
      ...
      <fieldset class="border-none p-0 m-0">
        ...
        <div class="flex flex-wrap gap-3" :class="{ 'opacity-50 cursor-not-allowed': isSubmitting }">
          <label v-for="slot in slots" :key="slot" :class="['cursor-pointer', { 'cursor-not-allowed': isSubmitting }]">
    ...
    <div class="space-y-2">
      ...
      <textarea id="message" v-model="form.message" name="message" rows="4" :disabled="isSubmitting" class="w-full border bg-gray-50 border-gray-200 focus-visible:ring-2 focus-visible:ring-accent focus:outline-none p-3 transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed"></textarea>
    </div>
