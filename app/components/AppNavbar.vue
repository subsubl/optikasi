<template>
  <nav class="fixed w-full z-50 transition-all duration-300 bg-white/90 backdrop-blur-md shadow-sm">
      ...
      <div class="hidden lg:flex space-x-8 items-center absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
        <div v-for="link in links" :key="link.path" class="relative group h-20 flex items-center">
          <NuxtLink :to="link.path"
            ...
            class="text-sm uppercase tracking-widest text-gray-600 hover:text-accent transition-colors font-medium h-full flex items-center focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-accent focus:outline-none rounded-sm">
            {{ link.name }}
          </NuxtLink>
           <!-- Dropdown -->
            <div v-if="link.children" class="absolute left-1/2 -translate-x-1/2 top-full w-72 bg-white shadow-lg py-2 border-t-2 border-primary opacity-0 invisible group-hover:opacity-100 group-hover:visible group-focus-within:opacity-100 group-focus-within:visible transition-all duration-300 transform origin-top-left z-50">
                <NuxtLink v-for="child in link.children" :key="child.path" :to="child.path" class="block px-6 py-3 text-xs text-gray-600 hover:text-primary hover:bg-gray-50 uppercase tracking-wider border-b border-gray-50 last:border-0 text-left focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-accent focus:outline-none rounded-sm">
                    {{ child.name }}
                </NuxtLink>
            </div>
        </div>
    ...
    <!-- Mobile Menu -->
    <div v-show="isOpen" class="lg:hidden bg-white border-t max-h-[80vh] overflow-y-auto">
